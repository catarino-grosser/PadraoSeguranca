...
export async function criarAlerta(btn) {
  ...
  try {
    ...
    // Dispara a notificação push em paralelo — se falhar, o alerta já está
    // salvo com segurança no Firestore, então não bloqueia nem trava a tela.
    callFunction("notificar-alerta", {
      condominioNome: condominio.nome,
      portariaNome: portaria.nome,
      prioridade,
      observacao
    }).catch((err) => console.error("notificar-alerta:", err));
  } catch (err) {
