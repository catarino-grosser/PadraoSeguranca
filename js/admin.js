...
export async function saveUserProfile(btn) {
  ...
  const isEditing = $("userUid").readOnly;
  if (!isEditing && state.users.some((u) => u.id === uid)) {
    toast("Já existe um funcionário com este UID. Use o botão Editar na lista para alterá-lo.");
    return;
  }
  ...
  try {
    ...
    toast("Erro ao salvar funcionário. Tente novamente.");
  } finally {
    setLoading(btn, false);
  }
}
